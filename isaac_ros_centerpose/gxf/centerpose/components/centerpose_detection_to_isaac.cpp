...
#include "centerpose_detection_to_isaac.hpp"
...
struct PointerView {
  ...
  PointerView(const std::vector<T>& vec) : data{vec.data()}, size{vec.size()} {}

  PointerView(gxf::Handle<gxf::Tensor> tensor)
...
gxf_result_t CenterPoseDetectionToIsaac::tick() {
  ...
  auto maybe_result =
      ToDetection3DList(
          ...
          .and_then([&]() { *maybe_detection3_d_list.value().timestamp = *input_timestamp; });
  if (!maybe_result) {
